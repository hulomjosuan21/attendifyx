...
export const studentTable = pgTable('students_table', {
    ...
    studentCreatedAt: timestamp('student_created_at',{withTimezone: true}).notNull().defaultNow(),
    studentUpdatedAt: timestamp('student_updated_at', {withTimezone: true}).$onUpdateFn(() => new Date()),
},(table) => [
    ...
    index('full_name_composite_idx').on(table.lastName, table.firstName, table.middleName),
])
